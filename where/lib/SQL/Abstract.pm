...
sub _where_hashpair_HASHREF {
  ...
  for my $orig_op (sort keys %$v) {
    ...
    #### <maj>
    #### handle functions (with args in parens) and other expression-related items
    elsif ( grep /^$op$/, @{$self->{functions}} ) {
      $self->_SWITCH_refkind($val, {
	ARRAYREF => sub {
	  ($sql, @bind) = $self->_where_func_ARRAYREF($op, $val);
	},
	HASHREF => sub {
	  ($sql, @bind) = $self->_where_func_HASHREF($op, $val);
	},
	SCALAR => sub {
	  $sql = "$op($val)";
	  @bind = ();
...
# function(args) only created via { -func => \@args } (ie, in a hash)
# assuming f and g are in the function list...
# f(g(x)) should proceed from { -f => { -g => 'x' } },
#  and also from { -f => [ { -g => 'x' } ] }
# may want f( a, g(b), c): use this: { -f => ['a', { -g => 'b' }, 'c'] }
#
# what about { -f => [ 'a', ['b', {-g => 'c'}], 'd']? what does the nested
#  arrayref mean?
# what about { -f => { -g => 'x', -h => 'y' } } ? (multiple keys in value hash)
# These should probably distribute OR and AND (resp) in a meaningful way - i.e.,
...
sub _where_func_ARRAYREF {
  ...
  foreach my $farg (@$val) {
    my ($s, @b);
    @b = ();
    $self->_SWITCH_refkind($farg, {
      ARRAYREF => sub {
	($s, @b) = $self->_where_ARRAYREF($farg);
      },
      HASHREF => sub {
